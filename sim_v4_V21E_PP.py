...
df_materiali_PP = df_PP[['Ordine','Materiale','Versione','CDD_mese_inizio_cardine','CDD_anno_inizio_cardine']]
df_materiali_PP.rename(columns = {'Versione':'Linea'}, inplace=True)
df_cadenza_PP = cad_PP

df_scheduling_PP = pd.DataFrame(columns = ['data','capacity','materiale','cap_residua','ID','Materiale'])
